...
aggregate_group <- function(data, tasks) {
  data %>%
    ...
    group_by(pseudonym) %>%
    summarize(
      rt_FU = mean(rt_FU, na.rm = TRUE),
      rt_BL = mean(rt_BL, na.rm = TRUE),
    ...
    mutate(
      ReactionTime_Change = rt_FU - rt_BL,
...
create_plots <- function(agg, med_type = "LEDD", group_name, color, bootstrap_seed = 123) {
  ...
  boot_corr <- bootstrap_correlation(x_var, agg$ReactionTime_Change)
  obs_corr <- cor(x_var, agg$ReactionTime_Change, use = "complete.obs")
